...
class RequestVersion2:
    ...
    def access_token_is_valid(self, access_token):
        """A simple test method to determine if an access_token is valid. No request payload.
        :return: True if the access_token is valid. False otherwise.
        """
        ...
        if access_token:
            r_type = 'POST'
            url = f'https://hotels.cloudbeds.com/api/v1.1/access_token_check'
            header = {'Authorization': 'Bearer ' + access_token}

            r = requests.request(r_type, url, headers=header)
            if r.status_code == 200:
                return True
            else:
                return False
    ...
    def get_access_token_from_code(self):
        ...
        response_in_json = self.basic_request(r_type, url, payload=payload)

        if 'access_token' in response_in_json.keys():
            self.write_json(response_in_json, self.TOKENS_PATH)
            return response_in_json['access_token'], response_in_json['refresh_token']
        else:
